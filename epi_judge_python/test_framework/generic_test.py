...
def generic_test_main(
    ...
    try:
        ...
        config = TestConfig.from_command_line(
            test_file,
            test_data_file,
            config_override["timeoutSeconds"],
            config_override["numFailedTestsBeforeStop"],
            commandline_args,
        )
        ...
        print(
            "\nCritical error({}): {}".format(e.__class__.__name__, e), file=sys.stderr
        )
        import traceback
...
def run_tests(handler, config, res_printer):
    ...
    for test_case in test_data[1:]:
        ...
        try:
            test_output = handler.run_test(
                config.timeout_seconds, config.metrics_override, test_case
            )
        ...
        if result != TestResult.PASSED:
            ...
            if test_explanation not in ("", "TODO"):
                test_failure.with_property(PropertyName.EXPLANATION, test_explanation)
            print_failed_test(
                handler.param_names(), test_case, test_failure, res_printer
            )
...
def update_test_passed(test_file, tests_passed):
    problem_mapping_file_path = get_file_path_in_judge_dir("problem_mapping.js")
    ...
    with open(problem_mapping_file_path) as problem_mapping_file:
        chapter_to_problem_to_language_solution_mapping = json.loads(
            problem_mapping_file.read()
            .replace(js_begin_pattern, "")
    ...
    for chapter in chapter_to_problem_to_language_solution_mapping.values():
        for _, language_solution_mapping in chapter.items():
            if test_file in language_solution_mapping:
                ...
                with open(problem_mapping_file_path, "w") as problem_mapping_file:
