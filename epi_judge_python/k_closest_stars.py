...
def find_closest_k_stars(stars: Iterator[Star], k: int) -> List[Star]:
    k_max_heap = []
    heapq.heapify(k_max_heap)
    for star in itertools.islice(stars, k):
        heapq.heappush(k_max_heap, (-star.distance, star))

    for star in stars:
        _, furthest_star = k_max_heap[0]
        if star < furthest_star:
            heapq.heappushpop(k_max_heap, (-star.distance, star))

    return [star for _, star in k_max_heap]
