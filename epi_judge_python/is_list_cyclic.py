...
def has_cycle(head: ListNode) -> Optional[ListNode]:
    ...
    while fast and fast.next:
        ...
        if slow is fast:
            # Narrow down the start point by "spinning" an interator around
            # to find the cycle length, and then starting two iterators separated
            # by the cycle length. The moment they intersect must be the start
            # of the cycle!
            # N.B.: for this test, it seems returning any node in the cycle is
            # enough :shrug:
            i, cyc_len = slow.next, 1
            while i is not slow:
                i, cyc_len = i.next, cyc_len + 1

            i = cycler = head
            for _ in range(cyc_len):
                cycler = cycler.next
            while i is not cycler:
                i, cycler = i.next, cycler.next
            return i
